...
# Auto-load from sample_documents directory
    ...
    if os.path.exists(sample_dir):
        for filename in os.listdir(sample_dir):
            if filename.endswith(".txt"):
                ...
                doc_id = hashlib.md5(filename.encode() + str(size).encode()).hexdigest()[:12]
                
                def _human_size_local(n: int) -> str:
                    if n < 1024: return f"{n} B"
                    if n < 1024 * 1024: return f"{n/1024:.1f} KB"
                    return f"{n/(1024*1024):.1f} MB"
                
                chunks = max(1, math.ceil(size / 500))
                
                # Avoid duplicates if already present
                if not any(d["name"] == filename for d in _documents):
                    _documents.append({
                        "id": doc_id,
                        "name": filename,
                        "size": size,
                        "size_human": _human_size_local(size),
                        "chunks": chunks,
                        "status": "Embedded",
                        "type": "txt",
                    })
