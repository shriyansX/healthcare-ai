...
export default function FileUpload({ onFilesAdded }) {
  ...
  const upload = async (file) => {
    ...
    try {
      ...
      onFilesAdded?.([{ name: data.name || file.name, size: data.size_human || humanSize(file.size), chunks: data.chunks || 5, status: data.status || 'Embedded', id: data.id || id, type: data.type || file.name.split('.').pop() }]);
    } catch (err) {
